...
function ProjectsGallery() {
    ...
    const [headers, setHeaders] = useState(["MetaBacTrack", "Respiratory activity", "ReadingDataBase", "Converter bot"]);
    ...
    const [images, setImages] = useState([mbtIcon, raIcon, rdbIcon, converterIcon]);
    const [circleColors, setCircleColors] = useState(["#DDDDDD", "#DDDDDD", "#DDDDDD", "#DDDDDD"]);
                ...
                <div className={style.gallery}>
                    <div className={"gallery_heads"}>
                        {headers.map((header, index) => (
                            <h2 key={index} className={"gallery_header"}
                                style={{display: index === currentIndex ? 'block' : 'none'}}>{header}</h2>
                        ))}
                    </div>
                    <div className={"gallery_pictures"}>
                        {images.map((image, index) => (
                            <div key={index} className={style.gallery_img}
                                 style={{display: index === currentIndex ? 'block' : 'none'}}>
                                <Image alt={`project_icon_${index}`} src={image as string} priority={false}/>
                            </div>
                        ))}
                    </div>
                    <div className={"gallery_desc"}>
                        {descriptions.map((desc, index) => (
                            <p key={index} className={"gallery_para"}
                               style={{display: index === currentIndex ? 'block' : 'none'}}>{desc}</p>
                        ))}
                        {stack.map((stack, index) => (
                            <p key={index} className={"gallery_para"}
                               style={{display: index === currentIndex ? 'block' : 'none'}}><span
                                className={style.gallery_desc_stack}>Stack:</span> {stack}</p>
                        ))}
                        {links.map((a, index) => (
                            <a key={index}
                               href={a}
                ...
                <div className={style.gallery_scroller}>
                    <div className={style.gallery_arrows}>
                        <button type="button" className={style.moveLeft} onClick={moveLeft}>
                            <Image className={`${style.mover}`} src={moveArrow as string} alt="moving Arrow"/>
                        </button>
                        <button type="button" className={style.moveRight} onClick={moveRight}>
                            <Image className={`${style.mover} ${style.rightMover}`} src={moveArrow as string}
                                   alt="moving Arrow"/>
                        </button>
                    </div>
                    <div className={style.gallery_pointer}>
                        <svg width="110" height="10" viewBox="0 0 110 10" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            {circleColors.map((color, index) => (
                                <circle key={index} cx={(index * 25) + 30} cy="5" r="5" fill={color}/>
                            ))}
                        </svg>
                    </div>
                </div>
