...
function TypingGreeting() {
  const { displayedText, imagesVisible } = useTypingEffectWithImages(
    text,
    25,
    7
  );
  ...
  return (
    <div
      className={`${local_style.greeting_cont} ${typing_style.typing_container} ${typing_style.container}`}
    >
      <div className={local_style.typing_img}>
        <Image
          alt="Lyceum picture"
          src={li7Ico as string}
          className={`${imagesVisible ? '' : local_style.hidden} ${local_style.li7}`}
        />
        <Image
          alt="IU picture"
          src={iuIco as string}
          className={`${imagesVisible ? '' : local_style.hidden} ${local_style.iu}`}
        />
      </div>
      <div
        className={`${typing_style.typing} ${local_style.typing_text_greeting}`}
      >
        {displayedText}
      </div>
    </div>
